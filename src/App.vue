...
const handleSend = async (customQuery?: string, isRetry = false) => {
  ...
  try {
    ...
    if (is429Error(err)) {
      ...
      isRateLimited.value = false
      errorMessage.value = err instanceof Error ? err.message : t('fetchAnswerError')
    }
  } finally {
...
<template>
  <main
    class="h-full w-full min-h-full bg-portfolio-dark text-gray-200 flex flex-col items-center justify-between antialiased selection:bg-portfolio-purple/30 selection:text-white overflow-hidden"
  >
    <!-- Scrollable Messages Container -->
    <div
      ref="scrollContainer"
      class="w-full flex-1 overflow-y-auto flex flex-col items-center p-4 sm:p-6 pb-2"
    >
      <div class="w-full max-w-3xl flex-1 flex flex-col gap-5">
        <!-- Developer Header -->
        <AppHeader />
    ...
    <!-- Bottom Command / Query Input Box -->
    <div
      class="w-full max-w-3xl p-4 sm:px-6 pt-2 pb-4 bg-linear-to-t from-portfolio-dark via-portfolio-dark to-transparent z-20 shrink-0"
    >
      <div
